...
const server = new FastMCP({
  name: "feedmob_femini_mcp_server",
  version: "1.0.0",
  instructions: `
...
server.addTool({
  ...
  execute: async (args, { log }) => {
    try {
      ...
      const apiUrl = `${AI_API_URL}/api/unstable/mcp/campaign_spends?${queryParams.toString()}`;
...
server.addTool({
  ...
  execute: async (args, { log }) => {
    try {
      ...
      const apiUrl = `${AI_API_URL}/api/unstable/entities/search?q=${q}`;
...
server.addResource({
  ...
  async load() {
    const response = await fetch("https://feedmob-testing.s3.us-east-1.amazonaws.com/mcp_server_guides/campaign-spends-api-guide.md");
