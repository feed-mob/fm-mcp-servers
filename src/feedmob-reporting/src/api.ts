...
export async function getKohoFinancialSingularReports(
  start_date: string,
  end_date: string
): Promise<any> {
  const urlObj = new URL(`${FEEDMOB_API_BASE}/ai/api/koho_financial_singular_api_reports`);
  ...
  try {
    ...
    if (responseData.status === 200 && responseData.data && Array.isArray(responseData.data)) {
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const filename = `koho_financial_singular_reports_${start_date}_to_${end_date}_${timestamp}.csv`;
      const csvFilePath = saveDataToCsv(responseData.data, filename);
      responseData.csv_file_path = csvFilePath;
    }
    ...
    console.error("Error fetching Koho Financial Singular reports:", error);
    if (error && typeof error === 'object' && 'response' in error) {
      const err = error as Record<string, any>;
      const status = err.response?.status;
      if (status === 401) {
    ...
    throw new Error('Failed to fetch Koho Financial Singular reports');
  }
}
...
export async function getClients(
  ...
  const urlObj = new URL(`${FEEDMOB_API_BASE}/ai/api/clients`);

  // Add optional parameter
  if (client_name !== undefined) {
    urlObj.searchParams.append('client_name', client_name);
  }
...
export async function getCampaigns(
  ...
  const urlObj = new URL(`${FEEDMOB_API_BASE}/ai/api/campaigns`);
...
export async function getVendors(
  ...
  const urlObj = new URL(`${FEEDMOB_API_BASE}/ai/api/vendors`);

  // Add optional parameter
  if (vendor_name !== undefined) {
    urlObj.searchParams.append('vendor_name', vendor_name);
  }
...
export async function getDirectSpendJobStats(
  client_id?: number,
  vendor_id?: number,
  click_url_id?: number
): Promise<any> {
  const urlObj = new URL(`${FEEDMOB_API_BASE}/ai/api/direct_spend_job_stats`);
...
export async function getApps(
  ...
  const urlObj = new URL(`${FEEDMOB_API_BASE}/ai/api/apps`);
...
export async function getYouappiReportsGeo(
  start_date: string,
  end_date: string
): Promise<any> {
  const urlObj = new URL(`${FEEDMOB_API_BASE}/ai/api/youappi_reports/geo`);
  urlObj.searchParams.append('start_date', start_date);
  urlObj.searchParams.append('end_date', end_date);
  ...
  try {
    ...
    if (responseData.data && Array.isArray(responseData.data) && responseData.data.length > 0) {
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const filename = `youappi_reports_geo_${start_date}_to_${end_date}_${timestamp}.csv`;
      const csvFilePath = saveDataToCsv(responseData.data, filename);
      responseData.csv_file_path = csvFilePath;
    }
    ...
    console.error("Error fetching YouAppi geo reports:", error);
    if (error && typeof error === 'object' && 'response' in error) {
      const err = error as Record<string, any>;
      const status = err.response?.status;
      if (status === 401) {
    ...
    throw new Error('Failed to fetch YouAppi geo reports');
  }
}
...
export async function getAppsflyerCohortRevenuesGeo(
  ...
  const urlObj = new URL(`${FEEDMOB_API_BASE}/ai/api/appsflyer_cohort_revenues/geo`);
  urlObj.searchParams.append('client_id', String(client_id));
  urlObj.searchParams.append('start_date', start_date);
  urlObj.searchParams.append('end_date', end_date);
...
export async function getAppsflyerInAppEventsRetargetReports(
  ...
  try {
    ...
    console.error("Error fetching AppsFlyer in-app events retarget reports:", error);
    if (error && typeof error === 'object' && 'response' in error) {
      const err = error as Record<string, any>;
      const status = err.response?.status;
      if (status === 401) {
    ...
    throw new Error('Failed to fetch AppsFlyer in-app events retarget reports');
  }
}

export async function getBeeswaxReports(
  start_date: string,
  end_date: string
): Promise<any> {
  const urlObj = new URL(`${FEEDMOB_API_BASE}/ai/api/beeswax_reports`);
  urlObj.searchParams.append('start_date', start_date);
  urlObj.searchParams.append('end_date', end_date);
  ...
  try {
    ...
    if (responseData.data && Array.isArray(responseData.data) && responseData.data.length > 0) {
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const filename = `beeswax_reports_${start_date}_to_${end_date}_${timestamp}.csv`;
      const csvFilePath = saveDataToCsv(responseData.data, filename);
      responseData.csv_file_path = csvFilePath;
    }
    ...
    console.error("Error fetching Beeswax reports:", error);
    if (error && typeof error === 'object' && 'response' in error) {
      const err = error as Record<string, any>;
      const status = err.response?.status;
      if (status === 401) {
    ...
    throw new Error('Failed to fetch Beeswax reports');
  }
}
...
export async function getRemergeReports(
  ...
  try {
    ...
    console.error("Error fetching Remerge reports:", error);
    if (error && typeof error === 'object' && 'response' in error) {
      const err = error as Record<string, any>;
      const status = err.response?.status;
      if (status === 401) {
